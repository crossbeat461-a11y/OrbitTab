...
function setupImportFeature() {
    ...
    const bgBtn = document.getElementById('bg-change-btn'), bgInput = document.getElementById('bg-input'), bgContainer = document.getElementById('bg-container');
...
function makeWidget(el, key, def) {
    ...
    header.onmousedown = function(e) {
        ...
        document.onmousemove = (me) => { el.style.left = (me.clientX - startX) + "px"; el.style.top = (me.clientY - startY) + "px"; };
...
function render() {
    ...
    Object.keys(links).forEach((cat, idx) => {
        ...
        box.innerHTML = `<div class="widget-header"><span class="widget-title">${cat}</span><div class="header-btns"><span class="cat-btn cat-delete-btn">🗑️</span></div></div><div class="link-list"></div>`;
        
        box.querySelector('.cat-delete-btn').onclick = () => {
            if(confirm(`カテゴリ「${cat}」を削除？`)) { delete links[cat]; saveLinks(); render(); }
        };
        ...
        links[cat].forEach((item, lIdx) => {
            ...
            row.querySelector('.delete-link-btn').onclick = (e) => {
                e.stopPropagation();
                if(confirm(`削除しますか？`)) { links[cat].splice(lIdx, 1); saveLinks(); render(); }
            };
...
function setupButtonActions() {
    ...
    document.getElementById('note-setup-btn').onclick = () => { notes.push({title: "新規メモ", body: ""}); saveNotes(); render(); };
}
...
window.onload = () => { 
    ...
    if (bg && bgContainer) {
        ...
        bgContainer.style.backgroundSize = "cover"; bgContainer.style.backgroundPosition = "center";
        bgContainer.style.backgroundRepeat = "no-repeat"; bgContainer.style.backgroundAttachment = "fixed";
    }
};